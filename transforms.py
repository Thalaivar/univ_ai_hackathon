...
# def segregate_income_groups(df:pd.DataFrame):
...
def make_biased_dataset(df):
    if "risk_flag" in df.columns:
        ...
        neg_idx = np.random.permutation(np.arange(y.size))[
            :int(psr * pos_idx.size)]
...
def convert_categorical(train_df, test_df=None, cols=[]):
    for col in cols:
...
def add_house_status(df: pd.DataFrame) -> pd.DataFrame:
    df["has_house"] = df["house_ownership"].apply(
        lambda s: int(s != "norent_noown"))
    return df
